...
/** To select only 2014 **/
data crimelow_population_2014;
	set summary_crime_population;
	where year=2014;
	crime_rate=total_crimes/new_population;
run;

proc sort data=crimelow_population_2014;
	by crime_rate;
run;
ods html path='/home/yeong_jin0/ODS Output/' 
         body='Top 3 Lowest Crimes.html';
/** To print the result for the Top 3 **/
title 'US Crime Stats : Top 3 States with Lowest Crime Rate (2014)';
proc print data=crimelow_population_2014(obs=3) label style(header)={just=c foreground=black}
...
data crimelow_population_2015;
	set summary_crime_population;
	where year=2015 and state in ('IDAHO','NEW YORK','NORTH DAKOTA');

	if		state='IDAHO'		then	rank=1;
	else if state='NEW YORK'	then	rank=2;
	else								rank=3;
	crime_rate=total_crimes/new_population;
run;

proc sort data=crimelow_population_2015;
  by rank;
run;

/** To print the result for the Top 3 **/
title1 'US Crime Stats : Top 3 States with Lowest Crime Rate in 2014';
title2 'Crime Rate in 2015';
proc print data=crimelow_population_2015(obs=3) label style(header)={just=c foreground=black}
	style(table)	=	{width=100%};
	format	new_population	comma14.
			total_crimes	comma14.
			crime_rate		percent7.2;
